...
def extract_tables_from_content_list(
    ...
    3. Как только начинается настоящий раздел примечаний
       ("ПРИМЕЧАНИЯ К ... 2024 ГОДА"), дальнейший контекстный поиск
       прекращается — принимаются только таблицы с точным caption.
    ...
    for block in blocks:
        ...
        if btype == "text":
            ...
            # Жёсткое начало примечаний (с годом в заголовке) → полная остановка
            if _NOTES_HARD_RE.search(txt) and len(txt) < 250:
                notes_started = True
                current_form = None
                continue

            # В разделе примечаний контекст больше не обновляем
            if notes_started:
                continue

            # Строки оглавления (". 6", ". .12") — пропускаем
            if _TOC_RE.search(txt):
                continue

            # Мягкий сброс: блок начинается со слова «Примечание/Примечания»
            if _NOTES_RE.search(txt):
                current_form = None
                continue

            # Форму определяем только из коротких заголовков (не из абзацев)
            ...
            # Caption — всегда; контекст — только вне раздела примечаний
            form_key = detect_form_from_caption(caption_text)
            if not form_key and not notes_started:
